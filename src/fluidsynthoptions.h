    ...
    strcat(shaderPathBuf, "GeneralUser_GS_FluidSynth_v1.44.sf2");

    Pstring = secprop->Add_path("fluidsynth.soundFontFile", Property::Changeable::OnlyAtStart, shaderPathBuf);
    Pstring->Set_help("File used as the soundfont for rendering MIDI music. A soundfont is a collection of digitized audio samples, each used to \"play\" a different musical instrument.");

    const char *fluidMIDIType[] = { "gm", "gs", "xg", "mma", 0};
    Pstring = secprop->Add_string("fluidsynth.midiType", Property::Changeable::OnlyAtStart, "gm");
    Pstring->Set_help("MIDI Type -- controls the interpretation of Bank Select. Options are \"gm\", \"gs\", \"xg\", and \"mma\".");
    Pstring->Set_values(fluidMIDIType);

    Pint = secprop->Add_int("fluidsynth.sampleRate", Property::Changeable::OnlyAtStart, 41000);
    Pint->Set_help("Sample rate -- Rate at which the synthesizer runs. Higher needs more CPU.");

    Pbool = secprop->Add_bool("fluidsynth.thread", Property::Changeable::OnlyAtStart, true);
    Pbool->Set_help("Whether to allow the synthesizer an additional thread");

    const char *fluidInterpVals[] = { "0", "1", "4", "7", 0};
    Pint = secprop->Add_int("fluidsynth.interpolation", Property::Changeable::OnlyAtStart, 1);
    Pint->Set_help("Sample interpolation method. Values are: 0=None(Fastest); 1=Linear(Fast); 4=4th-order(medium); 7=7th-order(slow). No other values are legal.");
    Pint->Set_values(fluidInterpVals);
...
Pbool = secprop->Add_bool("mt32.thread",Property::Changeable::WhenIdle,true);
